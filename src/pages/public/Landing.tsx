...
const DEMO_EMAILS: Record<'ADMIN' | 'USER' | 'SECURITY', string> = {
  ADMIN: 'admin@parksmart.edu',
  USER: 'jane.doe@parksmart.edu',
  SECURITY: 'security@parksmart.edu',
};
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      <nav className="p-6 flex justify-between items-center relative z-10 max-w-7xl mx-auto w-full">
        <div className="flex items-center gap-2">
          ...
          <span className="text-2xl font-bold text-white tracking-tight">ParkSmart</span>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 relative z-10 max-w-7xl mx-auto w-full">
        ...
        <div id="demo" className="w-full max-w-5xl pt-10 border-t border-white/10">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold text-white mb-4">Interactive Demo</h2>
            <p className="text-slate-400">
              Select a role below to jump straight into the system without logging in.
            </p>
          </div>

          <div className="grid md:grid-cols-3 gap-6">
            <GlassCard hoverEffect onClick={() => handleDemoLogin('USER')} className="text-center group cursor-pointer">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Student / Faculty</h3>
              <p className="text-sm text-slate-400">
                Find parking, make reservations, and manage digital permits.
              </p>
            </GlassCard>

            <GlassCard hoverEffect onClick={() => handleDemoLogin('SECURITY')} className="text-center group cursor-pointer">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Security Staff</h3>
              <p className="text-sm text-slate-400">
                Verify permits, log entries, and monitor violations.
              </p>
            </GlassCard>

            <GlassCard hoverEffect onClick={() => handleDemoLogin('ADMIN')} className="text-center group cursor-pointer">
        ...
        {/* Footer note */}
        <p className="mt-16 text-xs text-slate-600 text-center">
          ParkSmart © 2026 — Campus Parking Management System
        </p>
      </main>
    </div>
  );
...
const Badge = ({ children, className }: { children: React.ReactNode; className?: string }) => (
  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 ${className || ''}`}>
    {children}
  </span>
);
