...
export const UserDashboard = () => {
  ...
  const activeZone = activeReservation ?
  zones.find((z) => z.id === activeReservation.zoneId) :
  null;
  const activeSlot = activeZone ?
  activeZone.slots.find((s) => s.id === activeReservation?.slotId) :
  null;
  return (
        ...
        <div className="md:col-span-2">
          ...
          <GlassCard className="flex flex-col items-center justify-center text-center py-12 border-dashed border-white/20">
              ...
              <p className="text-slate-400 mb-6 max-w-md">
                You don't have any upcoming parking reservations. Book a spot
                now to guarantee parking.
              </p>
              <Button onClick={() => navigate('/user/map')}>
                Find Parking
              </Button>
            </GlassCard>
          }
          ...
          <GlassCard>
            ...
            <div className="space-y-3">
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg text-sm text-amber-200">
                <strong>High Demand:</strong> North Campus Lot is currently at
                95% capacity.
              </div>
              <div className="p-3 bg-white/5 rounded-lg text-sm text-slate-300">
                East Visitor Lot maintenance scheduled for tomorrow.
              </div>
            </div>
          </GlassCard>
        </div>
