...
export const ParkingMap: React.FC<ParkingMapProps> = ({
  ...
  const availableCount = selectedZone.slots.filter(
    ...
    <div className={`space-y-6 ${dimmed ? 'opacity-50 pointer-events-none select-none' : ''}`}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 hide-scrollbar">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl whitespace-nowrap font-medium transition-all ${selectedZoneId === zone.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/20' : 'bg-white/5 text-slate-300 hover:bg-white/10'}`}>
